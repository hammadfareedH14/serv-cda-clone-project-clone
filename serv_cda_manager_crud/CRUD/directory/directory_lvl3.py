...
# READ ALL 
@directory_blueprint.route('/directory/level3', methods=['GET'])
def get_directory_level3_items():
    items = DirectoryLevel3.get_all()
    return jsonify({"directory_level3_items": items}), 200

# CREATE 
@directory_blueprint.route('/directory/level3', methods=['POST'])
def create_directory_level3_item():
    ...
    directory_level3 = DirectoryLevel3(id, id_lv1, id_lv2, name, name_en, acronym, remark, instruction, status, value_min, 
                                       value_max, unit, price, display_order, call_name, ocare_code, order)
    directory_level3.save_to_db()

    return jsonify({"message": "Directory Level 3 item added successfully"}), 201

# READ  by ID
@directory_blueprint.route('/directory/level3/<item_id>', methods=['GET'])
...
# UPDATE 
@directory_blueprint.route('/directory/level3/<item_id>', methods=['PUT'])
...
# DELETE 
@directory_blueprint.route('/directory/level3/<item_id>', methods=['DELETE'])
